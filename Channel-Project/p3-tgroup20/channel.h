...
typedef struct {
    ...
    // Add bool to signal if this channel is open or closed
    bool is_open;

    // Add Mutex lock for updating shared variables (buffer, is_open)
    pthread_mutex_t lock;

    // Add condition variables
    // Need two, one to signal when we can take from buffer and one to signal when we can add to buffer
    pthread_cond_t condition_add;
    pthread_cond_t condition_take;

    // List of semaphore pointers registered by channel_select calls curently waiting on this channel
    // Used to wake up select callers when the channel state changes
    list_t* select_sems;
